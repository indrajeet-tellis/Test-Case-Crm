...
export function ProjectSummaryTabs({ projectStats }: { projectStats: any[] }) {
  ...
  return (
    <Tabs defaultValue={projectStats[0]?.id} className="w-full">
        ...
        <TabsList className="inline-flex h-auto w-auto min-w-full justify-start gap-2 bg-transparent p-0">
          {projectStats.map((project) => (
            <TabsTrigger
              key={project.id}
              value={project.id}
              className="rounded-lg border border-primary/20 bg-card/50 px-4 py-2 text-sm font-medium transition-all data-[state=active]:border-primary data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:shadow-[0_0_15px_rgba(255,0,255,0.2)] hover:bg-primary/5"
            >
              {project.name}
